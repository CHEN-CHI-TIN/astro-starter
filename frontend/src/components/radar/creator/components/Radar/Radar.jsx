...
const MAX_COLLISION_RETRY_COUNT = 3000;
...
function Radar(props) {
  ...
  // const toleranceX = (width / rings.length / 100) * TOLERANCE_CONSTANT * 2;
  // const toleranceY = props.fontSize || fontSize;
  // const toleranceX = 80;
  // const toleranceY = 80;

  const [toleranceX, setToleranceX] = useState(20);
  const [toleranceY, setToleranceY] = useState(20);
  ...
  const processRadarData = (quadrants, rings, data) => {
    ...
    for (const i in data) {
      ...
      for (let j = 0, len = quadrants.length; j < len; j++) {
        if (quadrants[j] === entry["radar_zone"]) {
      ...
      const blip = {
        id: entry["radar_zone"] + entry["innovation_tech_name"],
        name: entry["innovation_tech_name"],
        quadrant: entry["radar_zone"],
        cycleType: entry["radar_size"],
        ring: entry["radar_locate"],
        x: coordinates.x,
        y: coordinates.y,
      };
  ...
  const getRandomCoordinates = (
    ...
    const getPositionByQuadrant = (radiusArray) => {
      ...
      const margin = 0.2;
      const ringIndex = rings.indexOf(entry["radar_locate"]);
            ...
            return (
              <g key={index}>
                <Quadrant
                  ...
                  innovationIndex={props.innovationIndex}
                />
              </g>
            );
          })}
